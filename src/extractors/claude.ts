/**
 * Cloud engine via the Claude API (spec section 4, decision 5 — the paid,
...
export function createClaudeGenerate(config: Config): Generate {
  const apiKey = config.ANTHROPIC_API_KEY;
  if (!apiKey) {
    // loadConfig already enforces this; kept so the type narrows and so a
    // future caller cannot construct a keyless engine silently.
    throw new Error('ANTHROPIC_API_KEY is required for the claude engine');
  }

  // The JSON Schema in the request is not sent: the structured-output request
  // shape has not been verified against the live API, and the route's
  // validation is the gate either way.
  return async function generateWithClaude({ prompt, images }: GenerateRequest): Promise<unknown> {
    const content = [
      // Images before text: the model should see the page before the
    ...
    if (payload.stop_reason === 'refusal') {
      ...
      throw new ExtractorError('refused', `Claude declined the request (${category})`);
    }
